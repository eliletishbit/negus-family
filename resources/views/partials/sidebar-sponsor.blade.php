<aside class="sidebar-fixed">
    ...
    <div class="flex items-center gap-3 mb-8 pb-6 border-b border-[#1E293B]">
        ...
        <div>
            ...
            <p class="text-[#94A3B8] text-xs">Sponsor</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-6 p-3 rounded-xl bg-[#1E293B] border border-[#1E293B]">
        ...
        @else
            ...
            <div class="w-10 h-10 rounded-full bg-gradient-to-r from-[#D4AF37] to-[#E5C74A] flex items-center justify-center text-[#0F172A] font-bold">
                {{ substr(Auth::user()->nom ?? 'S', 0, 1) }}
            </div>
        @endif

        <div>
            <p class="text-white text-sm font-medium">{{ Auth::user()->nom ?? 'Sponsor' }}</p>
            <p class="text-[#94A3B8] text-xs">Sponsor</p>
        </div>
    </div>
    ...
    <nav class="space-y-1">
        ...
        {{-- Explorer --}}
        <a href="{{ route('sponsor.explorer') }}"
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all
                  {{ $active == 'explorer' ? 'bg-[#D4AF37]/10 text-[#D4AF37]' : 'text-[#94A3B8] hover:bg-[#1E293B] hover:text-white' }}">
            <i class="fa-solid fa-search w-5 text-center"></i>
            <span class="text-sm font-medium">Explorer</span>
        </a>

        {{-- Mes contacts débloqués --}}
        <a href="{{ route('sponsor.contacts') }}"
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all
                  {{ $active == 'debloques' ? 'bg-[#D4AF37]/10 text-[#D4AF37]' : 'text-[#94A3B8] hover:bg-[#1E293B] hover:text-white' }}">
            <i class="fa-solid fa-lock-open w-5 text-center"></i>
            <span class="text-sm font-medium">Mes contacts débloqués</span>
        </a>

        {{-- Portefeuille --}}
        <a href="{{ route('sponsor.portefeuille') }}"
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all
